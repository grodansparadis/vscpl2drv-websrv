...
CWebObj::doLoadConfig(void)
{
    ...
    if (m_j_config.contains("logging") && m_j_config["logging"].is_object()) {
        ...
        if (j.contains("file-level")) {
            ...
            if (std::string::npos != str.find("off")) {
                ...
                spdlog::error("Failed to read LOGGING 'file-level' has invalid value [{}]. Default value used.",
                                str);
            }            
        } 
        else  {
    ...
    if (m_j_config.contains("web") && m_j_config["web"].is_object()) {
        ...
        // document-root: .
        // A directory to serve. By default, the current working directory is served. The current 
        // directory is commonly referenced as dot (.). It is recommended to use an absolute path 
        // for document_root, in order to avoid accidentally serving the wrong directory.
        ...
        // Comma-separated list of ports to listen on. If the port is SSL, a letter s must be appended, 
        // for example, 80,443s will open port 80 and port 443, and connections on port 443 will be SSL-ed. 
        // For non-SSL ports, it is allowed to append letter r, meaning 'redirect'. Redirect ports will 
        // redirect all their traffic to the first configured SSL port. For example, if listening_ports is 
        // 80r,443s, then all HTTP traffic coming at port 80 will be redirected to HTTPS port 443.
        //
        // It is possible to specify an IP address to bind to. In this case, an IP address and a colon must 
        // be pre-pended to the port number. For example, to bind to a loopback interface on port 80 and to 
        // all interfaces on HTTPS port 443, use 127.0.0.1:80,443s.
        //
        // If the server is built with IPv6 support, [::]:8080 can be used to listen to IPv6 connections to 
        // port 8080. IPv6 addresses of network interfaces can be specified as well, e.g. [::1]:8080 for the 
        // IPv6 loopback interface.
        //
        // [::]:80 will bind to port 80 IPv6 only. In order to use port 80 for all interfaces, both IPv4 and
        // IPv6, use either the configuration 80,[::]:80 (create one socket for IPv4 and one for IPv6 only), 
        // or +80 (create one socket for both, IPv4 and IPv6). The + notation to use IPv4 and IPv6 will only 
        // work if no network interface is specified. Depending on your operating system version and IPv6 
        // network environment, some configurations might not work as expected, so you have to test to find 
        // the configuration most suitable for your needs. In case +80 does not work for your environment, 
        // you need to use 80,[::]:80.
        //
        // It is possible to use network interface addresses (e.g., 192.0.2.3:80, [2001:0db8::1234]:80). 
        // To get a list of available network interface addresses, use ipconfig (in a cmd window in Windows) 
        // or ifconfig (in a Linux shell). Alternatively, you could use the hostname for an interface. Check 
        // the hosts file of your operating system for a proper hostname (for Windows, usually found in 
        // C:\Windows\System32\drivers\etc, for most Linux distributions: /etc/hosts). E.g., to bind the IPv6 
        // local host, you could use ip6-localhost:80. This translates to [::1]:80. Beside the hosts file, 
        // there are several other name resolution services. Using your hostname might bind you to the 
        // localhost or an external interface. You could also try hostname.local, if the proper network 
        // services are installed (Zeroconf, mDNS, Bonjour, Avahi). When using a hostname, you need to 
        // test in your particular network environment - in some cases, you might need to resort to a fixed 
        // IP address.
        //
        // If you want to use an ephemeral port (i.e. let the operating system choose a port number), 
        // use 0 for the port number. This will make it necessary to communicate the port number to clients 
        // via other means, for example mDNS (Zeroconf, Bonjour, Avahi).
        //
        // In case the server has been built with the USE_X_DOM_SOCKET option set, it can listen to unix 
        // domain sockets as well. They are specified by a lower case x followed by the domain socket path, 
        // e.g. x/tmp/sockname. Domain sockets do not require a port number, always use HTTP (not HTTPS) and 
        // never redirect. Thus : is not allowed, while r or s at the end of the configuration is interpreted 
        // as part of the domain socket path. The domain socket path must be a valid path to a non-existing 
        // file on a Unix/Linux system. The CivetWeb process needs write/create access rights to create the 
        // domain socket in the Unix/Linux file system. Use only alphanumerical characters, underscore and / in 
        // a domain socket path (in particular, ,;: must be avoided).
        //
        // All socket/protocol types may be combined, separated by ,. E.g.: 127.0.0.1:80,[::1]:80,x/tmp/sockname 
        // will listen to localhost http connections using IPv4, IPv6 and the domain socket /tmp/sockname.
        ...
        // Authorization realm used for HTTP digest authentication. This domain is used in the encoding of
        // the .htpasswd authorization files as well. Changing the domain retroactively will render the 
        // existing passwords useless.
        ...
        // index-files: index.xhtml,index.html,index.htm,index.cgi,index.shtml,index.php
        // Comma-separated list of files to be treated as directory index files. If more than one matching 
        // file is present in a directory, the one listed to the left is used as a directory index.
        // 
        // In case built-in Lua support has been enabled, index.lp,index.lsp,index.lua are additional default 
        // index files, ordered before index.cgi.
        ...
        // When using absolute URLs, verify the host is identical to the authentication_domain. 
        // If enabled, requests to absolute URLs will only be processed if they are directed to 
        // the domain. If disabled, absolute URLs to any host will be accepted.
        ...
        // access-log-file : ""
        // Path to a file for access logs. Either full path, or relative to the current working directory. 
        // If absent (default), then accesses are not logged.
        ...
        // error-log-file: ""
        // Path to a file for error logs. Either full path, or relative to the current working directory. 
        // If absent (default), then errors are not logged.
        ...
        // throttle : Limit download speed for clients. throttle is a comma-separated list 
        ...
        // The value is a floating-point number of bytes per second, optionally followed 
        // by a k or m character, meaning kilobytes and megabytes respectively. A limit of 0 means 
        // unlimited rate. The last matching rule wins. 
        // Examples:
        //
        ...
        // Allows clients to reuse TCP connection for subsequent HTTP requests, which 
        // improves performance. For this to work when using request handlers it is important 
        // to add the correct Content-Length HTTP header for each request. If this is forgotten 
        // the client will time out.
        // 
        // Note: If you set keep_alive to yes, you should set keep_alive_timeout_ms to some 
        // value > 0 (e.g. 500). If you set keep_alive to no, you should set keep_alive_timeout_ms 
        // to 0. Currently, this is done as a default value, but this configuration is redundant. 
        // In a future version, the keep_alive configuration option might be removed and 
        // automatically set to yes if a timeout > 0 is set.
        ...
        // Idle timeout between two requests in one keep-alive connection. If keep alive is enabled, 
        // multiple requests using the same connection are possible. This reduces the overhead for 
        // opening and closing connections when loading several resources from one server, but it 
        // also blocks one port and one thread at the server during the lifetime of this connection. 
        // Unfortunately, browsers do not close the keep-alive connection after loading all resources 
        // required to show a website. The server closes a keep-alive connection, if there is no 
        // additional request from the client during this timeout.
        //
        // Note: if enable_keep_alive is set to no the value of keep_alive_timeout_ms should be set 
        // to 0, if enable_keep_alive is set to yes, the value of keep_alive_timeout_ms must be >0. 
        // Currently keep_alive_timeout_ms is ignored if enable_keep_alive is no, but future versions 
        // may drop the enable_keep_alive configuration value and automatically use keep-alive if 
        // keep_alive_timeout_ms is not 0.
        ...
        // access-control-list : "",
        // An Access Control List (ACL) allows restrictions to be put on the list of IP addresses 
        // which have access to the web server. In the case of the CivetWeb web server, the ACL is 
        // a comma separated list of IP subnets, where each subnet is pre-pended by either a - or a + sign. 
        // A plus sign means allow, where a minus sign means deny. If a subnet mask is omitted, such as -1.2.3.4, 
        // this means to deny only that single IP address.
        //
        // If this value is not set, all accesses are allowed. Otherwise, the default setting is to deny all 
        // accesses. On each request the full list is traversed, and the last match wins. Examples:
        //
        ...
        // See the Wikipedia page on Internet media types. Extension must include a leading dot. 
        ...
        //
        // If there are more simultaneous requests (connection attempts), they are queued. Every connection 
        // attempt first needs to be accepted (up to a limit of listen_backlog waiting connections). Then it is 
        // accepted and queued for the next available worker thread (up to a limit of connection_queue). Finally 
        // a worker thread handles all requests received in a connection (up to num_threads).
        //
        // In case the clients are web browsers, it is recommended to use num_threads of at least 5, since 
        // browsers often establish multiple connections to load a single web page, including all linked 
        // documents (CSS, JavaScript, images, ...).
        ...
        // Comma-separated list of URL rewrites in the form of uri_pattern=file_or_directory_path. When CivetWeb 
        // receives any request, it constructs the file name to show by combining document_root and the URI. 
        // However, if the rewrite option is used and uri_pattern matches the requested URI, then document_root 
        // is ignored. Instead, file_or_directory_path is used, which should be a full path name or a path relative 
        // to the web server's current working directory. Note that uri_pattern, as all CivetWeb patterns, is a 
        // prefix pattern.
        //
        // This makes it possible to serve many directories outside from document_root, redirect all requests to 
        // scripts, and do other tricky things. For example, to redirect all accesses to .doc files to a special 
        // script, do:
        //
        ...
        // hide-file-patterns : "",
        // A pattern for the files to hide. Files that match the pattern will not show up in directory listing 
        // and return 404 Not Found if requested. Pattern must be for a file name only, not including directory 
        // names. Example:
        //
        // CivetWeb -hide_files_patterns secret.txt|**.hide
        // Note: hide_file_patterns uses the pattern described above. If you want to hide all files with a 
        // certain extension, make sure to use **.extension (not just *.extension).
        ...
        // Timeout for network read and network write operations, in milliseconds. If a client intends to keep 
        // long-running connection, either increase this value or (better) use keep-alive messages.
        ...
        // decode-url : true
        // URL encoded request strings are decoded in the server, unless it is disabled by setting 
        // this option to no.
        ...
        // Path to a global passwords file, either full path or relative to the current working directory. 
        // If set, per-directory .htpasswd files are ignored, and all requests are authorized against that file.
        //
        ...
        // Password files may be generated using CivetWeb -A as explained above, or online tools e.g. this generator
        ...
        //
        // SSI is a simple interpreted server-side scripting language which is most commonly used to include 
        // the contents of another file in a web page. It can be useful when it is desirable to include a common 
        // piece of code throughout a website, for example, headers and footers.
        //
        // In order for a webpage to recognize an SSI-enabled HTML file, the filename should end with a special 
        // extension, by default the extension should be either .shtml or .shtm. These extensions may be changed 
        // using the ssi_pattern option.
        //
        // Unknown SSI directives are silently ignored by CivetWeb. Currently, two SSI directives are supported, 
        // <!--#include ...> and <!--#exec "command">. Note that the <!--#include ...> directive supports three 
        // path specifications:
        //
        ...
        // The exec directive is used to execute a command on a server, and show the output that would have been 
        // printed to stdout (the terminal window) otherwise. Example:
        //
        // <!--#exec "ls -l" -->
        ...
        // Access-Control-Allow-Origin header field, used for cross-origin resource sharing (CORS). 
        ...
        // Access-Control-Allow-Methods header field, used for cross-origin resource sharing (CORS) pre-flight 
        // requests. See the Wikipedia page on CORS.
        //
        // If set to an empty string, pre-flights will not be supported directly by the server, but scripts may 
        // still support pre-flights by handling the OPTIONS method properly. If set to "*", the pre-flight will 
        // allow whatever method has been requested. If set to a comma separated list of valid HTTP methods, the 
        // pre-flight will return exactly this list as allowed method. If set in any other way, the result is 
        // unspecified.
        ...
        // Access-Control-Allow-Headers header field, used for cross-origin resource sharing (CORS) pre-flight 
        // requests. See the Wikipedia page on CORS.
        //
        // If set to an empty string, pre-flights will not allow additional headers. If set to "*", the pre-flight 
        // will allow whatever headers have been requested. If set to a comma separated list of valid HTTP headers, 
        // the pre-flight will return exactly this list as allowed headers. If set in any other way, the result is 
        // unspecified.
        ...
        // error-pages : ""
        // This option may be used to specify a directory for user defined error pages. To specify a directory, 
        // make sure the name ends with a backslash (Windows) or slash (Linux, MacOS, ...). The error pages may be 
        // specified for an individual http status code (e.g., 404 - page requested by the client not found), a group 
        // of http status codes (e.g., 4xx - all client errors) or all errors. The corresponding error pages must be 
        // called error404.ext, error4xx.ext or error.ext, whereas the file extension may be one of the extensions 
        // specified for the index_files option. See the Wikipedia page on HTTP status codes.
        ...
        //
        // If set the socket option will disable Nagle's algorithm on the connection which means that packets 
        // will be sent as soon as possible instead of waiting for a full buffer or timeout to occur.
        // 
        ...
        // Set the Cache-Control header of static files responses. The string value will be used directly.
        //
        // E.g. this config:
        //
        ...
        //
        // This option will set the Cache-Control: max-age value for static files. Dynamically generated 
        // content, i.e., content created by a script or callback, must send cache control headers by themselves.
        //
        // A value >0 corresponds to a maximum allowed caching time in seconds. This value should not exceed one 
        // year (RFC 2616, Section 14.21). A value of 0 will send "do not cache at all" headers for all static files. 
        ...
        // Set the Strict-Transport-Security header, and set the max-age value. This instructs web browsers 
        // to interact with the server only using HTTPS, never by HTTP. If set, it will be sent for every 
        // request handled directly by the server, except scripts (CGI, Lua, ..) and callbacks. They must 
        // send HTTP headers on their own.
        //
        // The time is specified in seconds. If this configuration is not set, or set to -1, no 
        // Strict-Transport-Security header will be sent. For values <-1 and values >31622400, the 
        ...
        // It is only available for Linux systems and only affecting HTTP (not HTTPS) connections if 
        // throttle is not enabled. While using the sendfile call will lead to a performance boost for 
        // HTTP connections, this call may be broken for some file systems and some operating system versions.
        ...
        // additional-header : ""
        // Send additional HTTP response header line for every request. The full header line including 
        // key and value must be specified, excluding the carriage return line feed.
        //
        ...
        // Note: This example is valid, if the default configuration values for index_files, cgi_pattern 
        // and lua_script_pattern are used, and the server is built with CGI and Lua support enabled.
        //
        ...
        // will call /here/script.cgi with PATH_INFO='/handle/this.ext', no matter if this option is set 
        // to yes or no.
        //
        ...
        // A buffer of the configured size is pre allocated for every worker thread. max_request_size 
        // limits the HTTP header, including query string and cookies, but it does not affect the HTTP body 
        // length. The server has to read the entire header from a client or from a CGI script, before it is 
        // able to process it. In case the header is longer than max_request_size, the request is considered as 
        // invalid or as DoS attack. The configuration value is approximate, the real limit might be a few bytes 
        ...
        // If this feature is not activated, only the first file (/some/path/file.cgi) will be accepted.
        //
        // Note: This parameter affects only index scripts. A path like /here/script.cgi/handle/this.ext will 
        // call /here/script.cgi with PATH_INFO='/handle/this.ext', no matter if this option is set to yes or no.
        //
        // This feature can be used to completely hide the script extension from the URL.
        ...
        if (j.contains("tls") && j["tls"].is_object()) {
            ...
            // Certificate
            // Path to the SSL certificate file. This option is only required when at least one of the 
            // listening\_ports is SSL. The file must be in PEM format, and it must have both, private key and 
            // certificate, see for example ssl_cert.pem A description how to create a certificate can be found in 
            // doc/OpenSSL.md
            ...
            // CA Path
            // Name of a directory containing trusted CA certificates. Each file in the directory must 
            // contain only a single CA certificate. The files must be named by the subject name’s hash 
            // and an extension of “.0”. If there is more than one certificate with the same subject name 
            // they should have extensions ".0", ".1", ".2" and so on respectively.
            ...
            // Verify depth: 9
            // Sets maximum depth of certificate chain. If client's certificate chain is longer than the depth 
            // set here connection is refused.
            ...
            // Short trust: false
            // Enables the use of short lived certificates. This will allow for the certificates and keys 
            // specified in ssl_certificate, ssl_ca_file and ssl_ca_path to be exchanged and reloaded while 
            // the server is running.
            //
            // In an automated environment it is advised to first write the new pem file to a different filename 
            // and then to rename it to the configured pem file name to increase performance while swapping the 
            // certificate.
            //
            // Disk IO performance can be improved when keeping the certificates and keys stored on a tmpfs 
            // (linux) on a system with very high throughput.  
...
CWebObj::eventExToReceiveQueue(vscpEventEx& ex)
{
    ...
    if (!vscp_convertEventExToEvent(pev, &ex)) {
        spdlog::get("logger")->error(
               "Failed to convert event from ex to ev.");
    ...
    if (NULL != pev) {
        ...
        spdlog::get("logger")->error(
               "Unable to allocate event storage.");
    }
    return true;
}
