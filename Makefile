#
# Makefile : Builds vscpl2drv-template.so for Unix.
#
...
install: 
	...
	$(INSTALL_PROGRAM) install-files/vscpl2drv-template.conf $(DESTDIR)/vscpd
# Install sub components
	@for d in $(PROJ_SUBDIRS); do (echo "====================================================" &&\
	echo "Building in dir " $$d && echo "====================================================" && cd $$d && $(MAKE) install); done

uninstall: 
	rm $(DESTDIR)/vscpd/vscpl2drv-template.conf
