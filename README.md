...
<img src="https://vscp.org/images/logo.png" width="100">
    ...
    Driver Linux: vscpl2drv-template.so
    Driver Windows: vscpl2drv-template.dll
...
> sudo apt install ./vscpl2drv-template.deb
...
If you need to do dynamic configuration we recommend that you create the file in the */var/vscp/vscpl2drv-template.so*

A sample configuration file is make available in */usr/share/vscpl2drv-template.so* after installation.
...
```
git clone --recurse-submodules -j8 https://github.com/grodansparadis/vscpl2drv-template.so.git
cd vscpl2drv-template
./configure
make
make install
```
...
The VSCP daemon configuration is (normally) located at */etc/vscp/vscpd.conf*. To use the vscpl2drv-template.so driver there must be an entry in the
...
<driver enable="true"
    name="link"
    path-driver="/usr/lib/vscpl2drv-template.so"
    path-config="/var/lib/vscpl2drv-template/drv.conf"
...
##### path
This is the path to the driver. If you install from a Debian package this will be */usr/bin/vscpl2drv-template.so* and if you build and install the driver yourself it will be */usr/local/bin/vscpl2drv-template.so* or a custom location if you configured that.
...
#### vscpl2drv-template driver config
...
## Using the vscpl2drv-template driver
